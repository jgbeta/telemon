...
| `exporter_collector_samples` | gauge | `/metrics` | `linux_hwmon` | `collector`, `kind` | Useful sample count from the last collection run; currently `kind="temperature"`. |
| `system_uptime_seconds` | gauge | `/metrics` | `windows_baseline` | `source` | System uptime in seconds from Windows APIs. |
| `system_cpu_usage_ratio` | gauge | `/metrics` | `windows_baseline` | `component`, `source` | Total system CPU usage ratio from `0` to `1`; first sample appears after the second collection cycle. |
| `system_memory_bytes` | gauge | `/metrics`, `/metrics/static` | `windows_baseline` | `source`, `state` | Physical memory bytes; `state` is `total`, `available`, or `used`. `total` is static. |
...
| `system_os_info` | gauge | `/metrics/static` | `windows_inventory` | `source`, `os`, `version`, `build`, `arch` | Windows OS identity; value is always `1`. |
...
| `hardware_temperature_celsius` | gauge | `/metrics` | `linux_hwmon`, `nvidia_nvml`, `windows_lhm_http`, `windows_lhm_wmi` | `component`, `device_id`, `sensor`, `sensor_instance`, `source`, `source_driver`, optional source-specific labels | Dynamic hardware temperatures. Linux hwmon and Windows LHM use canonical sensor names such as `cpu_package_temp`, `cpu_core_temp`, `nvme_composite_temp`, `gpu_edge_temp`, and `vrm_temp`. |
...
| `hardware_device_info` | gauge | `/metrics/static` | `linux_hwmon`, `nvidia_nvml` | `component`, `device_id`, `source`, `source_driver`, source-specific identity labels | Hardware identity information for GPUs and NVMe storage; value is always `1`. Serial numbers are not exposed as Prometheus labels. |
...
| `hardware_voltage_volts` | gauge | `/metrics` | `windows_lhm_http` | `component`, `device_id`, `sensor`, `sensor_instance`, `source`, `source_driver` | Voltage readings from LibreHardwareMonitor HTTP. |
...
| `hardware_power_watts` | gauge | `/metrics` | `nvidia_nvml`, `windows_lhm_http` | `component`, `device_id`, `sensor`, `sensor_instance`, `source`, `source_driver`, optional `gpu_index` | Current hardware power usage in watts. |
| `hardware_power_limit_watts` | gauge | `/metrics/static` | `nvidia_nvml`, `windows_lhm_http` | `component`, `device_id`, `sensor`, `source`, `source_driver`, `limit`, optional `gpu_index` | Static-ish enforced/current power limit in watts. |
| `hardware_clock_hertz` | gauge | `/metrics` | `nvidia_nvml`, `windows_lhm_http` | `component`, `device_id`, `sensor`, `clock`, `source`, `source_driver`, optional `gpu_index` | Hardware clock speed in hertz. |
| `hardware_utilization_ratio` | gauge | `/metrics` | `nvidia_nvml`, `windows_lhm_http` | `component`, `device_id`, `sensor`, `engine`, `source`, `source_driver`, optional `gpu_index` | Utilization ratio from `0` to `1`. |
| `hardware_memory_bytes` | gauge | `/metrics`, `/metrics/static` | `nvidia_nvml`, `windows_lhm_http` | `component`, `device_id`, `memory`, `state`, `source`, `source_driver`, optional `gpu_index` | Hardware memory bytes; GPU VRAM uses `memory="vram"` and `state` values `total`, `used`, and `free`. |
...
| `hardware_fan_speed_rpm` | gauge | `/metrics` | `linux_hwmon`, `windows_lhm_http` | `component`, `device_id`, `sensor`, `sensor_instance`, `source`, `source_driver` | Fan or pump speed in RPM; `0` can be a valid stopped reading. |
| `hardware_state` | gauge | `/metrics` | `nvidia_nvml` | `component`, `device_id`, `sensor`, `state`, `source`, `source_driver`, `gpu_index` | Numeric hardware state values such as NVIDIA P-state where `P0` is exported as `0`. |
