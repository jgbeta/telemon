...
| `game_frame_source_supported` | gauge | `/fps` | `steam_deck_fps` | `source`, `queue` | `1` when Telemon opened a candidate frame timing queue. |
...
| `game_frame_source_samples_total` | counter | `/fps` | `steam_deck_fps` | `source`, `queue` | Total accepted frame timing samples. |
| `game_frame_source_dropped_total` | counter | `/fps` | `steam_deck_fps` | `source`, `queue`, `reason` | Frame timing samples dropped by sanity filters. Reasons: `zero`, `too_large`, `invalid_sentinel`, `unsupported_version`, `too_short`. |
