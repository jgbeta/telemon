...
| `hardware_clock_hertz` | gauge | `/metrics` | `nvidia_nvml`, `windows_lhm_http`, `macos_macmon` | `component`, `source`, optional `device_id`, `sensor`, `clock`, `unit`, `cluster`, `source_driver`, `gpu_index` | Hardware clock speed in hertz. `macos_macmon` converts CPU/GPU MHz values to hertz. |
| `hardware_utilization_ratio` | gauge | `/metrics` | `nvidia_nvml`, `windows_lhm_http`, `macos_macmon` | `component`, `source`, optional `device_id`, `sensor`, `engine`, `unit`, `cluster`, `source_driver`, `gpu_index` | Utilization ratio from `0` to `1`. |
